...
The following code runs the full Super Learner with a reduced library of candidate learners. This is for illustration only --- we recommend using a large library of candidate learners, as we did in the original analysis. The code to reproduce the results of the original analysis is provided in the Appendix, and was designed to run on a high-performance cluster (HPC) computer (it takes multiple hours to run).
...
fits <- parallel::mclapply(seeds, FUN = run_cv_sl_once, Y = Y_vaccine, X_mat = X_vaccine, family = "binomial",
              ...
              sl_lib = SL_library_with_assay_groups[2], 
...
We use the `vimp` package to compute variable importance estimates and confidence intervals. In all cases, we consider variable importance as a summary of the true data-generating distribution. Since our outcome is binary, we estimate variable importance in two ways: using the population difference in R-squared [@williamson2017], and using the population difference in area under the receiver operating characteristic curve (AUC). We interpret the R-squared-based importance as the proportion of the variability in the outcome explained by including a group of covariates. We interpret the AUC-based importance as the difference in our ability to discriminate between cases and controls based on using a group of covariates. Load the `vimp` package using the following code:

```{r load-vimp}
# only run the next two lines if you haven't installed vimp already
# install.packages("devtools")
# devtools::install_github("vimp")  # installing from GitHub to get latest release
library("vimp")
```

We next ran multiple Super Learners with reduced sets of covariates. Again, we originally ran this code on an HPC computer, and recommend that any final analyses use the full library of candidate learners. However, in the code below, we use a smaller library for illustration.
...
We ran the Super Learner algorithm for each of the 11 variable sets defined in Section \ref{sec:intro} using the procedure defined in Section \ref{sec:sl}. We present the cross-validated AUC and the cross-validated $R^2$ for the overall Super Learner from each of the variable sets. These are provided in Figures \ref{fig:cv_auc} and \ref{fig:cv_r2}, respectively. In each figure, the error bars are colored by immunoassay set: antibody (Ab) variables (black), no markers (yellow), T cell variables (green), and T cell and Ab variables (blue).

In Figure \ref{fig:cv_auc}, we see improved performance of the Super Learner when including marker variables compared to baseline risk variables only. Indeed, the Super Learner using only baseline variables and no markers has a cross-validated AUC of 0.58, and is the second-lowest cross-validated AUC: this confirms our hypothesis that using vaccine response marker variables improves prediction performance. The ordering of predictive performance fits our hypothesis that multiple types of immune response markers improve prediction: using no markers results in the poorest prediction performance (though the group of functional antibodies does not appear to result in any improvement), while using larger sets of markers tends to result in improved prediction performance. Our proposed estimator of HIV-1 infection risk among vaccinees, the cross-validated Super Learner, has an estimated AUC of 0.748 based on all markers, with a 95\% confidence interval of [0.697, 0.799]. This is a marked improvement over the Super Learner based on only the baseline risk variables, with an estimated AUC of 0.58 [0.517, 0.643].

AUC FIGURE HERE

In Figure \ref{fig:cv_r2}, we see that there is substantial estimated variability in the cross-validated $R^2$ values: this is due to the small variance of the outcome in these data, resulting in efficient influence function-based standard errors that are large relative to the point estimates. We see that, in general, the cross-validated $R^2$ is not large for any variable set, with the largest cross-validated $R^2$ estimated to be 0.11 when only all markers are considered.

R-SQUARED FIGURE HERE


### Variable Importance Analysis

We estimated the variable importance for 10 groups: all marker variables, IgG + IgA, IgG3, T cells, functional antibodies, IgG + IgA and IgG3, IgG + IgA and T cells, IgG + IgA and IgG3 and T cells, IgG + IgA and IgG3 and functional antibodies, and T cells and functional antibodies. In each case, we used the cross-validated Super Learner predictions from the analysis based on using the appropriate set of marker variables and baseline risk variables as our estimate of the full regression function; we then used the cross-validated Super Learner based on the baseline risk variables only to define the reduced regression function. This defines variable importance as the improvement in prediction performance from including a group of marker variables relative to including baseline variables only. 

The results based on cross-validated AUC are presented in Figure \ref{fig:vimp_auc}. Here, we see that using the group of all marker variables results in an estimated increase in cross-validated AUC of nearly 0.2 over using the baseline risk variables only. Interestingly, the groups of IgG, IgA, IgG3, and T cell markers; IgG, IgA, and T cell markers; and all marker variables have estimated equivalent prediction performance. The next most important group appears to be those markers measured in T cells and functional antibody assays, with an estimated increase in cross-validated AUC of approximately 0.15 compared to using baseline risk variables only. Marker variables measured by T cell assays may be the most important individual group of markers, judging from the large importance of the T cells group alone and the fact that any group containing T cells has large estimated importance relative to the baseline risk variables. The analysis also shows that both antibodies and T cells are needed to optimize predictions: performance substantially improves adding binding antibodies (IgG, IgA, IgG3) to T cells. Adding all of the distinct antibody variable groups improves prediction performance compared to leaving any of the antibody groups out of the prediction algorithm. 
