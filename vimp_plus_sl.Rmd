...
1. Assay type: CD4 T Cell, CD8 T Cell, IgG, IgA, functional (antibody-dependent cellular phagocytosis [ADCP; referred to as \code{phago}], Fc$\gamma$ receptors [referred to as fcrR2a and fcrR3a], and neutralizing antibody [nAb] tier 1)
...
```{r run-full-sl-with-small-lib, cache = TRUE}
...
## on your computer
# install.packages("~/Software/vimp_2.0.0.tar.gz", type = "source", repos = NULL)
library("vimp")
```
...
```{r run-assays-with-small-lib, cache = TRUE}
...
for (i in (1:14)[-11]) {
  ...
  set.seed(4747)
  seeds <- round(runif(10, 1000, 10000)) # average over 10 random starts
  fits <- parallel::mclapply(seeds, FUN = run_cv_sl_once, Y = Y_vaccine, X_mat = X_vaccine,
                             family = "binomial", obsWeights = weights_vaccine,
                             sl_lib = sl_lib, 
                             method = "method.CC_nloglik",
                             cvControl = list(V = V_outer, stratifyCV = TRUE),
                             innerCvControl = list(list(V = V_inner)),
...
for (i in 1:(length(var_set_names))) { 
  ...
  eval(parse(text = paste0("all_aucs_i <- tibble::as_tibble(do.call(rbind.data.frame, lapply(sl_fits_varset_", var_set_names[i], ", function(x) get_all_aucs_lst(x, weights = weights_vaccine))))")))
...
for (i in 1:length(var_set_names)) {
  this_name <- paste(unlist(strsplit(var_set_names[i], "_", fixed = TRUE))[-1], collapse = "_")
  eval(parse(text = paste0("all_r2s_i <- tibble::as_tibble(do.call(rbind.data.frame, lapply(sl_fits_varset_", var_set_names[i], ", function(x) get_all_r2s_lst(x, weights = weights_vaccine))))")))
  all_r2s_i <- all_r2s_i %>% 
