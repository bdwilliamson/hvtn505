...
```{r test}
avg_aucs <- tibble(Learner = NA, varset_label = NA, AUC = NA, 
                   ci_ll = NA, ci_ul = NA)
vimp_tibble <- tibble(assay_grp = NA, est = NA, 
                      cil = NA, ciu = NA)
```
...
```{r run-full-sl, code = readLines("code/run_sl.R")}
```

Code to reproduce the full variable importance analysis:
```{r run-assays, code = readLines("code/run_sl_assays.R")}
```
