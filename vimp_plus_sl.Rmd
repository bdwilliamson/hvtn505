...
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 70, indent = 2), tidy = 'styler')
```
...
We used two layers of nested cross-validation in this analysis. To obtain cross-validated estimates of risk, we used an outer layer of five-fold cross-validation stratified so that an equal proportion of events fell within each fold. To obtain estimates of the regression functions within each outer fold, we used an inner layer of leave-one-out cross-validation. We used the cross-validated negative log likelihood to determine our optimal convex combination of learners. Since the results may depend on the random number seed used to generate the folds for the outer layer of five-fold cross-validation, we used five random seeds (for illustration; in the main analysis, we used ten random seeds), and for each seed ran the full nested cross-validation procedure.

Our resulting proposed estimator for objective (1) above is the cross-validated Super Learner averaged over the five random seeds, where each prediction is based on a leave-one-out cross-validated Super Learner when the particular observation was in the outer validation fold.
...
```{r run-full-sl-with-small-lib, code = readLines("code/run_sl_vignette.R"), eval = FALSE}
```
...
```{r run-assays-with-small-lib, code = readLines("code/run_sl_assays_vignette.R"), eval = FALSE}
```

```{r read-sl-assays, echo = FALSE}
var_set_names <- c("1_baseline_exposure", "2_igg_iga", "3_igg3", 
                   ...
                   "14_igg3_tcells_fxab")
for (nm in var_set_names) {
  eval(parse(text = paste0(
    "sl_fits_varset_", nm, " <- readRDS('results/vignette_sl_fits_varset_", nm, ".rds')"
  )))
}
```
...
for (i in 1:(length(var_set_names))) {
  eval(parse(
    text = paste0("all_aucs_i <- as_tibble(rbindlist(lapply(sl_fits_varset_",
                  var_set_names[i],
                  ", function(x) x$aucs)))")))
    ...
    eval(parse(text = 
    paste0("avg_aucs_", var_set_names[i], " <- all_aucs_i %>%
...
avg_aucs <- bind_rows(avg_aucs_1_baseline_exposure, avg_aucs_2_igg_iga, 
                      avg_aucs_3_igg3,
...
```{r run-full-sl, code = readLines("code/run_sl.R"), eval = FALSE}
```

Code to reproduce the full variable importance analysis:
```{r run-assays, code = readLines("code/run_sl_assays.R"), eval = FALSE}
```
