...
plot_assays <- function(avgs = NULL, type = "auc", main_font_size_forest = 5, 
  ...
  top_learner_plot <- top_learners %>% 
    ggplot(aes(y = measure, x = factor(paste0(Screen, "_", Learner, "_", assay), levels = paste0(Screen, "_", Learner, "_", assay)[order(measure)],
                                   labels = paste0(varset_label, " ", learner_nm, " ", screen_nm)[order(measure)]),
               xend = factor(paste0(Screen, "_", Learner, "_", assay), levels = paste0(Screen, "_", Learner, "_", assay)[order(measure)],
                             labels = paste0(varset_label, " ", learner_nm, " ", screen_nm)[order(measure)]))) + 
    geom_errorbar(aes(ymin = ci_ll, ymax = ci_ul), size = point_size) +
    geom_pointrange(aes(ymin = ci_ll_truncated, ymax = ci_ul_truncated), size = point_size) +
    ylab(x_lab) +
    xlab("") + 
