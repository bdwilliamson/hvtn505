...
r2_plot_assays <- function(avg_r2s, main_font_size) {
  ...
  top_learners_labs$y_coord <- rep(as.numeric(rownames(top_learners))[order(top_learners$R2, decreasing = FALSE)], dim(top_learners_labs)[1]/length(as.numeric(rownames(top_learners))))
