...
one_auc <- function(preds, Y, scale = "logit",
  ...
  se <- vimp::vimp_se(auc_lst$point_est, auc_lst$eif, scale = scale)
...
#            and may include control parameters for the super learner 
#            (e.g., cvControl = list(V = 5) for 5-fold cross-validated super learner)
get_all_aucs <- function(sl_fit, scale = "logit", weights = rep(1, length(sl_fit$Y)), C = rep(1, length(sl_fit$Y)),
                         Z = NULL, ...) {
  ...
  get_individual_auc <- function(sl_fit, col, scale = "logit", weights = rep(1, length(sl_fit$Y)),
...
#            and may include control parameters for the super learner 
#            (e.g., cvControl = list(V = 5) for 5-fold cross-validated super learner)
get_all_aucs_lst <- function(sl_fit_lst, scale = "logit",
...
#            and may include control parameters for the super learner 
#            (e.g., cvControl = list(V = 5) for 5-fold cross-validated super learner)
one_r2 <- function(preds, Y, scale = "logit", weights = rep(1, length(Y)), 
                   C = rep(1, length(Y)), Z = NULL, ...) {
  r2 <- vimp::measure_r_squared(fitted_values = preds, y = Y, C = C, Z = Z, 
                                ipc_weights = weights, ipc_fit_type = "SL", ...)
  se <- vimp::vimp_se(r2$point_est, r2$eif, scale = scale)
...
cv_r2 <- function(preds, Y, folds, scale = "logit", weights = rep(1, length(Y)), 
                  C = rep(1, length(Y)), Z = NULL, ...) {
  V <- length(folds)
  fold_row_nums <- as.vector(do.call(cbind, folds))
  folds_init <- rep(as.numeric(names(folds)), each = length(Y)/length(folds))
...
get_all_r2s <- function(sl_fit, scale = "logit", weights = rep(1, length(sl_fit$Y)), 
                        C = rep(1, length(sl_fit$Y)), Z = NULL, ...) {
  ...
  get_individual_r2 <- function(sl_fit, col, scale = "logit", 
...
get_all_r2s_lst <- function(sl_fit_lst, scale = "logit", weights = rep(1, length(sl_fit_lst[[1]]$fit$Y)), 
