...
SL_library_with_assay_groups <- sapply(
  1:length(methods),
  function(m) c(methods[m], screens_with_assay_groups),
  simplify = FALSE
)
SL_library <- sapply(
  1:length(methods),
  function(m) c(methods[m], screens),
  simplify = FALSE
)
# -----------------------------------------------------------------------------
...
test_with_assay_groups <- c(
  apply(screen_method_frame_with_assay_groups, 1, paste0, collapse = "_")
  )
test <- c(apply(screen_method_frame, 1, paste0, collapse = "_"))
